...
class PhysicsInformedUNet1D(nn.Module):
    """
    ┌──────────────────────────────────────────────────────────────────────────────────────┐
    │ Physics-Informed 1-D U-Net                                                           │
    │ ──────────────────────────────────────────────────────────────────────────────────── │
    │ • Enhanced with physics-informed features as additional input channels               │
    ...
    def __init__(self, in_channels=13, out_channels=2, features=[32, 64, 128, 256], dropout=0.1):
        super().__init__()
        ...
        # Physics-informed feature weighting - will be initialized properly later
...
# ═══════════════════════════════════════════════════════════════════════════════════════
# 🏗️ ENHANCED DATASET WITH PHYSICS-INFORMED FEATURES
# ═══════════════════════════════════════════════════════════════════════════════════════

class PhysicsInformedSeismicDataset(Dataset):
    """
    ┌──────────────────────────────────────────────────────────────────────────────────────┐
    │ Physics-Informed Seismic Dataset                                                     │
    │ ──────────────────────────────────────────────────────────────────────────────────── │
    │ • Loads raw seismic waveforms                                                        │
    │ • Computes physics-informed features (STA/LTA, envelope, spectral)                   │
    │ • Creates multi-channel input combining raw data with traditional features           │
    ...
    def __init__(self, manifest, window_size=5, target_length=None):
        ...
        for i, item in enumerate(manifest):
            try:
                if self._process_waveform(item):
                    successful_loads += 1
                    
                if (i + 1) % 10 == 0:
                    print(f"Processed {i + 1}/{len(manifest)} waveforms, "
                          f"successful: {successful_loads}")
                    
            except Exception as e:
                print(f"Error processing waveform {i}: {e}")
                continue
...
def train_model(model, train_loader, val_loader, num_epochs=50, learning_rate=0.001):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter = 0
            # Save best model
            torch.save(model.state_dict(), 'best_physics_informed_model.pth')
        else:
            patience_counter += 1
...
def visualize_physics_features(dataset, sample_idx=0, save_path='physics_features.png'):
    """Visualize the physics-informed features for a sample"""
...
def main():
    ...
    earthquakes = [
        ...
        #{"time": "2021-12-20T20:10:31", "lat": 40.390, "lon": -124.298, "radius": 100},  # Petrolia
        #{"time": "2021-07-08T22:49:48", "lat": 38.508, "lon": -119.500, "radius": 100},  # Antelope
        #{"time": "2020-04-11 14:36:37", "lat": 38.053, "lon": -118.733, "radius": 100},  # Bodie
    ]
    ...
    # Load best model
    try:
        model.load_state_dict(torch.load('best_physics_informed_model.pth'))
    ...
    plt.tight_layout()
    plt.savefig('physics_informed_training_results.png', dpi=150, bbox_inches='tight')
    plt.show()
    ...
    # Save model
    torch.save(model.state_dict(), 'physics_informed_phase_picker.pth')
    print(f"\n💾 Model saved as 'physics_informed_phase_picker.pth'")
